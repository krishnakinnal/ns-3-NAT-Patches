...
//                              192.168.1.5  203.82.48.1   203.82.48.2
 ...
 //nat->AddAddressPool (Ipv4Address ("202.82.43.0"), Ipv4Address ("202.82.43.5"), Ipv4Mask ("255.255.255.0"));
  nat->AddAddressPool ("202.101.43.0", "0.0.0.4","0.0.0.6", "255.255.255.0");
 ...
 clientApps = echoClient.Install (nodes.Get (4));
  ...
  pointToPoint.EnablePcapAll ("ipv4-pat", false);

  Simulator::Run ();
  // Now print them out
  Ptr<OutputStreamWrapper> natStream = Create<OutputStreamWrapper> ("pat.rules", std::ios::out);
